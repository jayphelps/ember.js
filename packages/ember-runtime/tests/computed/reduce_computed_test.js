...
QUnit.test('doubly nested item property keys (@each.foo.@each) are not supported', function() {
  run(function() {
    obj = EmberObject.createWithMixins({
      peopleByOrdinalPosition: Ember.A([{
        first: Ember.A([EmberObject.create({
...
QUnit.module('arrayComputed - changeMeta property observers', {
  setup() {
    ...
    run(function() {
      obj = EmberObject.createWithMixins({
        items: Ember.A([EmberObject.create({
          n: 'zero'
        }), EmberObject.create({
          n: 'one'
        })]),
...
QUnit.test('changeMeta includes item and index', function() {
  ...
  // add2
  run(function() {
    items.pushObject(EmberObject.create({
      n: 'two'
    }));
  });
