...
QUnit.test('it maps simple unshifted properties', function() {
  ...
  run(function() {
    obj = EmberObject.createWithMixins({
      array: array,
      mapped: computedMap('array', function(item) {
...
QUnit.test('it passes the index to the callback', function() {
  ...
  run(function() {
    obj = EmberObject.createWithMixins({
      array: array,
      mapped: computedMap('array', function(item, index) {
        return index;
      })
    });
    get(obj, 'mapped');
...
QUnit.test('it maps objects', function() {
  deepEqual(get(obj, 'mappedObjects'), [{
    name: 'Robert'
  }, {
    name: 'Leanna'
  }]);
  ...
  run(function() {
    obj.get('arrayObjects').objectAt(0).set('v', {
      name: 'Stannis'
    });
  });
...
QUnit.test('it passes the index to the callback', function() {
  ...
  run(function() {
    obj = EmberObject.createWithMixins({
      array: array,
      filtered: computedFilter('array', function(item, index) {
        return index === 1;
      })
    });
    get(obj, 'filtered');
...
QUnit.test('it passes the array to the callback', function() {
  ...
  run(function() {
    obj = EmberObject.createWithMixins({
      array: array,
      filtered: computedFilter('array', function(item, index, array) {
