...
QUnit.test('chained observers on enumerable properties are triggered when the observed property of any item changes', function() {
  var family = EmberObject.create({
    momma: null
  });
  var momma = EmberObject.create({
    children: []
  });

  var child1 = EmberObject.create({
    name: 'Bartholomew'
  });
  var child2 = EmberObject.create({
    name: 'Agnes'
  });
  var child3 = EmberObject.create({
    name: 'Dan'
  });
  var child4 = EmberObject.create({
    name: 'Nancy'
  });
  ...
  run(function() {
    get(momma, 'children').setEach('name', 'Juan');
  });
